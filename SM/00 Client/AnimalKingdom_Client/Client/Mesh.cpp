...
CMesh::CMesh( const CMesh& Instance )
	...
	, m_ePrimitiveTopology( Instance.m_ePrimitiveTopology )
...
void CMesh::Render( ID3D11DeviceContext* pContext, UINT dwInstanceCnt/* = 1*/ )
{
	UINT dwOffset{ 0 };
	pContext->IASetPrimitiveTopology( m_ePrimitiveTopology );

	if( nullptr != m_pVB )
		pContext->IASetVertexBuffers( 0, 1, &m_pVB, &m_dwVtxSize, &dwOffset );

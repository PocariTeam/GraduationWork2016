...
VS_OUT VS( VS_IN In )
{
	...
	Out.vWorldPos = mul( float4( In.vPos, 1.f ), In.mtxWorld ).xyz;
	Out.vUV = In.vUV;

	Out.vNormal = normalize( mul( float4( In.vNormal, 0.f ), In.mtxWorld ) ).xyz;
	Out.vTangent = normalize( mul( float4( In.vTangent, 0.f ), In.mtxWorld ) ).xyz;
	Out.vOption = In.vOption;
...
PS_OUT PS( VS_OUT In )
{
	...
	Out.vDepth = float4( In.vPos.z / In.vPos.w, In.vPos.w * 0.001f, In.vOption.x, 0.f );

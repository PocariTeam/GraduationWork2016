...
// 뷰를 병합하는 과정, 뷰포트를 설정하는 과정은 없음 ( 아직 )
...
CGraphicDev* CGraphicDev::Create( WINMODE Mode, HWND hWnd, const WORD& wSizeX, const WORD& wSizeY )
{
	...
	if( FAILED( pGraphicDev->Ready_GraphicDev( Mode, hWnd, wSizeX, wSizeY ) ) )
	{
		pGraphicDev->Release();
		pGraphicDev = nullptr;
...
HRESULT CGraphicDev::Ready_GraphicDev( WINMODE Mode, HWND hWnd, const WORD& wSizeX, const WORD& wSizeY )
{
	DWORD dwCreateDeviceFlag = 0;

#if defined(DEBUG) || defined(_DEBUG)
	// vs 2012 깔면됨
	// dwCreateDeviceFlag |= D3D11_CREATE_DEVICE_DEBUG;	/* 디버그 모드에는 반드시 ( 플래그 사용시에는 Win8의 SDK 필요_SDKLayer.dll ) */
#endif
	...
	DWORD dwDriverTypes = sizeof( DriverTypes ) / sizeof( D3D10_DRIVER_TYPE );

	/* 사용자의 하드웨어가 어떤 버젼에 대응되는 기능 수준인가 */
	/* 현재코드에서는 사용중이지 않음 */
	...
	DWORD dwFeatureLevels = sizeof( FeatureLevels ) / sizeof( D3D_FEATURE_LEVEL );
	...
	for( DWORD i = 0; i < dwDriverTypes; i++ )
	{
		...
		if( SUCCEEDED( D3D11CreateDevice( NULL /* 기본어댑터를 사용하겠다 */
			...
			, dwCreateDeviceFlag	/* 위에 참조 */
			, 0	/* 항상 FeatureLevel 이 11.0 일 때만 ( NULL을 넣을 시 최고수준을 찾아줌 ) */
			, 0 /* 앞의 인자인 FeatureLevel 변수의 원소 개수 ( NULL 일 때는 0 ) */
			, D3D11_SDK_VERSION
			, &m_pDevice
	...
	if( FAILED( Create_StencilView( wSizeX, wSizeY ) ) ) {
		MessageBoxA( 0, "StencilView 생성 실패", 0, 0 );
		return E_FAIL;
	}

	m_pDeviceContext->OMSetRenderTargets( 1, &m_pRenderTargetView, m_pStencilView );
...
HRESULT CGraphicDev::Create_StencilView( const WORD& wSizeX, const WORD& wSizeY )
{
	...
	if( FAILED( m_pDevice->CreateTexture2D( &DepthBufferDesc, NULL, &m_pStencilBuffer ) ) )
		return E_FAIL;

	if( FAILED( m_pDevice->CreateDepthStencilView( m_pStencilBuffer, NULL, &m_pStencilView ) ) )
		return E_FAIL;
...
void CGraphicDev::BeginScene( void )
{
	...
	m_pDeviceContext->ClearDepthStencilView( m_pStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL
		, 1.f, 0 );
}
...
void CGraphicDev::Clear_DeathBuffer( float fInit )
{
	m_pDeviceContext->ClearDepthStencilView( m_pStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL
		, fInit, 0 );
}
...
#define			OFFSET  NxVec3( -300.f, 0.f, 0.f )
...
void SwitchCoordinateSystem( NxScene* scene, NxMat34 mat )
{
	...
	for( NxU32 i = 0; i < nbActors; i++ )
	{
		...
		actorpos = MtxChangeLook*actorpos;

		if( !actor->isDynamic() )
			bStaticActor = true;

		if( !bStaticActor )
			actor->setGlobalPosition( actorpos + OFFSET );
		...
		for( NxU32 j = 0; j < nbShapes; j++ )
		{
			...
			shapepos = shape[ j ]->getLocalPosition();
			shapepos = MtxChangeLook * shapepos;
			shape[ j ]->setLocalPosition( NxVec3( 0.f, 0.f, 0.f ) );
		...
		if( bStaticActor )
		{
			// 현재 맵은 회전의 값을 갖으므로 임시방편
			if( 0 == strcmp( "map", actor->getName() ) )
				newActorDesc.globalPose.t = actorpos + NxVec3( -1.f * shapepos.x, shapepos.z, shapepos.y ) + OFFSET;				
			else
				newActorDesc.globalPose.t = actorpos + shapepos + OFFSET;
			gScene->createActor( newActorDesc );

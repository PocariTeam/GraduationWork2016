...
void CAnimator::ConnectActorShape( Player* pOwner )
{
	...
	for( NxU32 j = 0; j < dwActorCnt; ++j )
	{
		...
		XMMATRIX mtxResult = XMMatrixMultiply( CMathematics::ConvertToXMMatrix( &pOwner->getCCT()->getActor()->getGlobalPose() ), XMMatrixMultiply( mtxLoadAnimation, mtxLoadOrigin ) );
